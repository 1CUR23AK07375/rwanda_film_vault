...
# ============================================================
# Home Page (Visitor tracking + Filters + Trending/New)
# ============================================================
def home(request):
    # session tracking
    ...
    visitor.save(
        update_fields=["country", "city", "lat", "lng", "last_visit", "visit_count"]
    )
    ...
    remaining_qs = base_qs.exclude(
        id__in=(trending_ids + new_ids)
    ).order_by(*main_ordering)
...
def watch_movie(request, movie_id):
    ...
    total_views = (
        movie.total_views
        if hasattr(movie, "total_views")
        else WatchHistory.objects.filter(movie=movie).count()
    )
...
def real_time_viewers(request, movie_id):
    active_window = timezone.now() - timedelta(minutes=ACTIVE_WINDOW_MINUTES)
    count = WatchHistory.objects.filter(
        movie_id=movie_id, end_time__isnull=True, start_time__gte=active_window
    ).count()
...
def _is_online(last_watch):
    if not last_watch:
        return False
    return (
        last_watch.end_time is None
        and last_watch.start_time
        and (timezone.now() - last_watch.start_time <= timedelta(minutes=ACTIVE_WINDOW_MINUTES))
    )
...
def visitor_stats_api(request):
    ...
    for v in visitors:
        last_watch = (
            WatchHistory.objects.filter(ip_address=v.ip_address)
        ...
        rows.append(
            {
                ...
                "visit_count": WatchHistory.objects.filter(
                    ip_address=v.ip_address
                ).count(),
                "last_visit": v.last_visit.strftime("%Y-%m-%d %H:%M:%S")
                if v.last_visit
                else "",
                "last_movie": last_watch.movie.name
                if last_watch and last_watch.movie
                else "-",
            }
        )
...
def search_suggestions(request):
    q = request.GET.get('q', '')
    matches = Movie.objects.filter(name__icontains=q)[:5]  # return top 5 matches
    results = [{'id': m.id, 'name': m.name} for m in matches]
    return JsonResponse(results, safe=False)
def latest_movies(request):
    # Fetch movies uploaded in the last 10 minutes (adjust as needed)
